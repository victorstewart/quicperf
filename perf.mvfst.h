...
public:
	MvfstNetworkSocket(NetworkHub<mode> *hub, std::shared_ptr<quic::QuicEventBase> eventBase)
		: networkHub(hub), evb(std::move(eventBase)), localAddress(mvfstSocketAddressFromSockaddr(hub->socket.address()))
	{
	}

	quic::Expected<void, quic::QuicError> init(sa_family_t) override { return ok(); }
	...
	quic::Expected<void, quic::QuicError> connect(const folly::SocketAddress&) override { return ok(); }
	quic::Expected<void, quic::QuicError> close() override { return ok(); }
	...
	void pauseRead() override { readPaused = true; }
	...
	void pauseWrite() override { writeCallback = nullptr; }
	bool isWritableCallbackSet() const override { return writeCallback != nullptr; }
	...
	quic::Expected<void, quic::QuicError> setGRO(bool) override { return ok(); }
	quic::Expected<void, quic::QuicError> setRecvTos(bool) override { return ok(); }
	quic::Expected<bool, quic::QuicError> getRecvTos() override { return false; }
	quic::Expected<void, quic::QuicError> setTosOrTrafficClass(uint8_t) override { return ok(); }
	quic::Expected<folly::SocketAddress, quic::QuicError> address() const override { return localAddress; }
	const folly::SocketAddress& addressRef() const override { return localAddress; }
	void attachEventBase(std::shared_ptr<quic::QuicEventBase> eventBase) override { evb = std::move(eventBase); }
	void detachEventBase() override { evb.reset(); }
	std::shared_ptr<quic::QuicEventBase> getEventBase() const override { return evb; }
	quic::Expected<void, quic::QuicError> setCmsgs(const folly::SocketCmsgMap&) override { return ok(); }
	quic::Expected<void, quic::QuicError> appendCmsgs(const folly::SocketCmsgMap&) override { return ok(); }
	quic::Expected<void, quic::QuicError> setAdditionalCmsgsFunc(std::function<quic::Optional<folly::SocketCmsgMap>()>&&) override { return ok(); }
	quic::Expected<int, quic::QuicError> getTimestamping() override { return -1; }
	quic::Expected<void, quic::QuicError> setReuseAddr(bool) override { return ok(); }
	quic::Expected<void, quic::QuicError> setDFAndTurnOffPMTU() override { return ok(); }
	quic::Expected<void, quic::QuicError> setErrMessageCallback(ErrMessageCallback *) override { return ok(); }
	quic::Expected<void, quic::QuicError> applyOptions(const folly::SocketOptionMap&, folly::SocketOptionKey::ApplyPos) override { return ok(); }
	quic::Expected<void, quic::QuicError> setReusePort(bool) override { return ok(); }
	quic::Expected<void, quic::QuicError> setRcvBuf(int) override { return ok(); }
	quic::Expected<void, quic::QuicError> setSndBuf(int) override { return ok(); }
	quic::Expected<void, quic::QuicError> setFD(int, FDOwnership) override { return ok(); }
	int getFD() override { return networkHub->socket.fd; }
	quic::QuicAsyncUDPSocket::ReadCallback *storedReadCallback() const { return readCallback; }
};
...
public:
	...
	void onTransportReady() noexcept override { transportReady = true; }
	void onReplaySafe() noexcept override { handshakeDone = true; }
	void onFullHandshakeDone() noexcept override { handshakeDone = true; }
	...
	void onConnectionEnd() noexcept override { connectionEnded = true; }
	void onConnectionEnd(quic::QuicError) noexcept override { connectionEnded = true; }
...
private:
	...
	quic::TransportSettings transportSettings(void) const
	{
		...
		settings.defaultCongestionController = strcmp(benchmarkCongestionProfile, "none") == 0
	...
	{
		auto cert = fizz::openssl::CertUtils::makeSelfCert(mvfstReadFile(tls_cert), mvfstReadFile(tls_key));
		...
		context->setSupportedSigSchemes({
	...
	std::shared_ptr<quic::FizzClientQuicHandshakeContext> makeClientContext(void)
	{
		...
		fizzClientContext->setSupportedSigSchemes({
			fizz::SignatureScheme::ed25519,
			fizz::SignatureScheme::ecdsa_secp256r1_sha256,
			fizz::SignatureScheme::rsa_pss_sha256});
		...
		std::shared_ptr<fizz::CertificateVerifier> verifier =
			...
			? std::shared_ptr<fizz::CertificateVerifier>(
				fizz::openssl::OpenSSLCertificateVerifier::createFromCAFile(
					fizz::VerificationContext::Client, tls_chain).release())
			...
			case ServerPhase::readRequest:
			{
				...
				if (streamState.requestRead == streamState.request.size())
				{
					...
					streamState.payloadRemaining = (benchmarkScenario == BenchmarkScenario::multistream_upload ||
						benchmarkScenario == BenchmarkScenario::bidi) ? streamState.requestValue : 0;
	...
	void runClientGeneric(uint64_t bytes)
	{
		...
		while (completed < targetStreams)
		{
			while (opened < targetStreams && active.size() < maxActive)
			{
				...
				active.push_back(makeGenericClientStream(*result, opened, bytes));
				++opened;
			}
