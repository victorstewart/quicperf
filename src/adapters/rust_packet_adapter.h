...
class RustPacketAdapter final : public Adapter {
public:
  RustPacketAdapter(uint32_t libraryKind, std::string library);
  ~RustPacketAdapter() override;
...
private:
  ...
  bool completeOperation(int status, uint64_t nowRawNs, AdapterError& error);
  PrimitiveStatus completePrimitiveOperation(int status, uint64_t nowRawNs,
                                             AdapterError& error);
  static qpf_addr_t toFfi(const sockaddr_in& address);
  static sockaddr_in fromFfi(const qpf_addr_t& address);
  static void assignError(const qpf_adapter_status_v2_t& status, AdapterError& error);
  ...
  uint32_t libraryKind_;
  Capabilities capabilities_;
  EndpointConfig config_ {};
  bool configured_ = false;
  qpf_engine_t* engine_ = nullptr;
  uint64_t nextTimeoutRawNs_ = 0;
  std::array<std::array<uint8_t, maxUdpPayloadSize>, packetBatchSize> output_ {};
  std::array<qpf_transmit_descriptor_v2_t, packetBatchSize> transmit_ {};
  std::array<qpf_receive_descriptor_v2_t, packetBatchSize> receive_ {};
