...
The data is fresh and complete for the configured run, but the strict publication gate rejected most rows for loopback noise, nonstationarity, or unsupported capability cases. Treat `publication-results.tsv`, `row-stats.tsv`, and `adaptive-run-summary.md` as the source of truth for row status.
