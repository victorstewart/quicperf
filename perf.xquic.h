...
private:
	...
	static ssize_t writeSocket(const unsigned char *buf, size_t size, const struct sockaddr *peerAddr, socklen_t peerAddrLen, void *userData)
	{
		...
		void openMoreGenericClientStreams(void)
		{
			...
			{
				if (!genericStarted || !benchmarkScenarioIsGenericStreamWorkload(benchmarkScenario) || !connected || closed)
				{
					return;
				}
	...
	static void saveString(const char *data, size_t dataLen, void *userData)
	{
	}
	...
	void createEngine(void)
	{
		...
		transportCallbacks.save_session_cb = saveString;
		transportCallbacks.save_tp_cb = saveString;
